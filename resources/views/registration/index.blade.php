...
@section('title', __('app.pra_registration'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold mb-0">
        <i class="fas fa-clipboard-list text-success me-2"></i>{{ __('app.pra_registration') }}
    </h2>
</div>

<div class="alert alert-info">
    <i class="fas fa-info-circle me-1"></i>
    {{ __('app.register_your_team') }}
</div>
...
@if($competitions->isEmpty())
    <div class="card">
        <div class="card-body text-center py-5">
            ...
            <h5 class="text-muted">{{ __('app.no_competitions_available') }}</h5>
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <ul class="list-unstyled small mb-3">
                            ...
                            <li class="mb-1">
                                ...
                                {{ $competition->teams_count }} {{ __('app.teams_registered') }}
                            </li>
                            <li class="mb-1">
                        ...
                        <div class="mt-auto">
                            <div class="mb-3">
                                <table class="table table-sm table-borderless mb-0">
                                    <tr>
                                        <td class="text-muted small">{{ __('app.yuran_penyertaan') }}</td>
                                        <td class="text-end fw-bold">
                                            @if($competition->registration_fee > 0)
                                                RM {{ number_format($competition->registration_fee, 2) }}
                                            @else
                                                <span class="text-success">{{ __('app.free') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small">{{ __('app.deposit_keselamatan') }}</td>
                                        <td class="text-end fw-bold">
                                            @if($competition->security_deposit > 0)
                                                RM {{ number_format($competition->security_deposit, 2) }}
                                            @else
                                                <span class="text-muted">{{ __('app.tiada') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted small">{{ __('app.bayaran_hari_perlawanan') }}</td>
                                        <td class="text-end fw-bold">
                            ...
                            <a href="{{ route('registration.create', $competition->id) }}" class="btn btn-success w-100">
                                <i class="fas fa-pen-to-square me-1"></i> {{ __('app.register_now') }}
                            </a>
                        </div>
                    </div>
                </div>
