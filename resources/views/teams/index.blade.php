...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold mb-0">
        <i class="fas fa-shield-halved text-success me-2"></i>Teams
    </h2>
    @auth
        <a href="{{ route('teams.create') }}" class="btn btn-success">
            <i class="fas fa-plus me-1"></i> Register Team
        </a>
    @endauth
</div>
                ...
                <tbody>
                    @foreach($teams as $team)
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ route('teams.show', $team) }}" class="btn btn-sm btn-outline-primary" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>
                                @auth
                                    @if(auth()->user()->isSuper() || auth()->user()->isLeagueAdmin())
                                        <a href="{{ route('teams.edit', $team) }}" class="btn btn-sm btn-outline-warning" title="Edit">
